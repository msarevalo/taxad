...
<div class="container">
        <form action="{{route('menu.editar', $menu->id)}}" method="post">
                    ...
                    <select class="form-control mb-2" name="padre" id="padre" required style="text-transform: capitalize">
                        ...
                        @foreach($padres as $padre)
                        	@if($padre->nombre!="Cerrar Sesión")
                        		@if($menu->menu_padre==$padre->id)
                        			<option style="text-transform: capitalize" value="{{$padre->id}}" selected>{{$padre->nombre}}</option>
                        		@else
                        			<option style="text-transform: capitalize" value="{{$padre->id}}" >{{$padre->nombre}}</option>
                        		@endif
                        	@endif
                        @endforeach
                    </select>
	                        ...
	                        @foreach($grupos as $grupo)
	                        	<optgroup label="{{$grupo->nombre}}">
	                        		@foreach($iconos as $icono)
	                        			@if($grupo->id==$icono->grupo)
		                        			@if($icono->class==$menu->class)
		                        				<option style="text-transform: capitalize" value="{{$icono->class}}" selected>{{$icono->nombre}}</option>
		                        			@else
		                        				<option style="text-transform: capitalize" value="{{$icono->class}}">{{$icono->nombre}}</option>
		                        			@endif
		                        		@endif
	                        		@endforeach
	                        	</optgroup>
        	...
        	<div class="form-group row">
                ...
                <div class="col-md-6">
                    <input id="orden" type="number" class="form-control @error('orden') is-invalid @enderror" name="orden" required autocomplete="orden" autofocus value="{{$menu->orden}}">
